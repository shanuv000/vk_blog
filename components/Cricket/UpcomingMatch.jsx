...
const UpcomingMatch = () => {
  ...
  // Update headings when upcomingMatches changes
  useEffect(() => {
    const updateHeadings = () => {
      if (upcomingMatches && upcomingMatches.length > 0) {
        const uniqueHeadings = Array.from(
          new Set(upcomingMatches.map((match) => match.heading))
        );
  ...
  // Filter upcoming matches based on selected heading
  const filteredMatches =
    upcomingMatches?.filter((match) => match.heading === selectedHeading) || [];
  ...
  return (
    upcomingMatches.length >= 1 && (
      ...
      >
        ...
        {upcomingMatchesError ? (
          <p className="text-red-200">{upcomingMatchesError}</p>
        ) : upcomingMatches.length === 0 && loadingUpcomingMatches ? (
